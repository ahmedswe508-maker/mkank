...
function InfoCard({ icon, label, value, href }: {
  icon: ReactNode; label: string; value: string; href: string
}) {
  return (
    <a
      href={href}
    ...
    <section id="contact" className="py-20 sm:py-24 bg-canvas">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14">
          <p className="text-secondary font-semibold text-xs uppercase tracking-widest mb-2">{t.contact.subtitle}</p>
          <h2 className="text-3xl sm:text-4xl font-black text-body">{t.contact.title}</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-start">
          ...
          {/* Form */}
          <form onSubmit={submit} className="bg-card rounded-2xl p-8 border border-edge shadow-sm space-y-5">
            {sent && (
            ...
            <div>
              <label className="block text-sm font-semibold text-body mb-1.5">{t.contact.name}</label>
              <input type="text" required value={form.name} onChange={set('name')}
                placeholder={t.contact.name} className={inputCls} />
            </div>

            <div>
              <label className="block text-sm font-semibold text-body mb-1.5">{t.contact.email}</label>
              <input type="email" required value={form.email} onChange={set('email')}
                placeholder={t.contact.email} className={inputCls} />
            </div>

            <div>
              <label className="block text-sm font-semibold text-body mb-1.5">{t.contact.message}</label>
              <textarea required rows={4} value={form.message} onChange={set('message')}
                placeholder={t.contact.message} className={`${inputCls} resize-none`} />
            </div>
